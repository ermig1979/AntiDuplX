...
ADX_VERSION_H="$ROOT_DIR/src/AntiDuplX/AdxVersion.h"
ADX_VERSION_H_TXT="$ROOT_DIR/prj/txt/AdxVersion.h.txt"
...
if [ ! -f "$ADX_VERSION_H" ]; then 
	NEED_TO_UPDATE="1"
fi

if [ "$NEED_TO_UPDATE" == "0" ] 
then
	if [ "$PRINT_INFO" != "0" ]; then echo "Skip updating of '$ADX_VERSION_H' file because there are not any changes."; fi
else
	if [ "$PRINT_INFO" != "0" ]; then echo "Create or update '$ADX_VERSION_H' file."; fi
	cp $ADX_VERSION_H_TXT $ADX_VERSION_H
	sed "-i" "s/@VERSION@/$FULL_VERSION/g" $ADX_VERSION_H
fi
